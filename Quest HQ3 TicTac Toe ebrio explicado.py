...
# Esta función evalúa el tablero recursivamente para encontrar el mejor movimiento.
# - es_maximizador: True si es turno de la IA (maximiza), False si es turno del jugador (minimiza).
# - simbolo_ia: Símbolo de la IA ('X' o 'O').
# - Devuelve: 1 si IA gana, -1 si jugador gana, 0 si empate, o el valor evaluado.
def minimax(tablero, profundidad, es_maximizador, simbolo_ia):
    ...
    if es_maximizador:
        # Turno de la IA: busca el movimiento que maximice su puntuación
        max_eval = -float('inf')
        for i in range(3):
            for j in range(3):
                if tablero[i][j] == ' ':
                    tablero[i][j] = simbolo_ia  # Simula el movimiento
                    eval = minimax(tablero, profundidad + 1, False, simbolo_ia)  # Llama recursivamente para el turno del jugador
                    tablero[i][j] = ' '  # Deshace el movimiento
                    max_eval = max(max_eval, eval)  # Elige el máximo
        return max_eval
    else:
        # Turno del jugador: busca el movimiento que minimice la puntuación de la IA
        min_eval = float('inf')
        for i in range(3):
            for j in range(3):
                if tablero[i][j] == ' ':
                    tablero[i][j] = simbolo_jugador  # Simula el movimiento
                    eval = minimax(tablero, profundidad + 1, True, simbolo_ia)  # Llama recursivamente para el turno de la IA
                    tablero[i][j] = ' '  # Deshace el movimiento
                    min_eval = min(min_eval, eval)  # Elige el mínimo
        return min_eval

# Función para el mejor movimiento de la IA
# Usa minimax para evaluar cada movimiento posible y elige el mejor.
def mejor_movimiento(tablero, simbolo_ia):
    ...
    for i in range(3):
        for j in range(3):
            if tablero[i][j] == ' ':
                tablero[i][j] = simbolo_ia  # Simula el movimiento
                valor_mov = minimax(tablero, 0, False, simbolo_ia)  # Evalúa con minimax (desde el turno del jugador)
                tablero[i][j] = ' '  # Deshace el movimiento
                if valor_mov > mejor_valor:
                    mejor_valor = valor_mov
                    mejor_mov = (i, j)  # Guarda el mejor movimiento
    return mejor_mov
...
class TriquiApp:
    ...
    def jugar_ia(self):
        if not self.turno_jugador:
            # Aquí se usa mejor_movimiento, que a su vez llama a minimax para elegir el movimiento óptimo
